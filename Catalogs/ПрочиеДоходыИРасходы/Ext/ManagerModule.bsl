...
Функция МассивСубконтоРазрешенныхПрочихДоходовИРасходов() Экспорт
	...
	Попытка
		...
		СсылкаНаПредопределенное = ПредопределенноеЗначение("Справочники.ПрочиеДоходыИРасходы.ВозвратТоваровПрошлыхЛет");
	Исключение
		СсылкаНаПредопределенное = Неопределено;
	КонецПопытки;
	Если ЗначениеЗаполнено(СсылкаНаПредопределенное) Тогда
		МассивСсылок.Добавить(СсылкаНаПредопределенное);
	КонецЕсли;
