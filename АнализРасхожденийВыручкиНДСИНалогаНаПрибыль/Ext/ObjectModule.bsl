...
Процедура ДополнитьПоказателиУчетнымиДаннымиАктуальныеДанныеДекларацииПоПрибыли(Организация, Показатели, СтрокаПериода)
	...
	ИсключаемыеВидыПрочихДоходовРасходов = Новый Массив;
	ИсключаемыеВидыПрочихДоходовРасходов.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияОсновныхСредств);
	ИсключаемыеВидыПрочихДоходовРасходов.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияНематериальныхАктивов);
	Запрос.УстановитьПараметр("ИсключаемыеВидыПрочихДоходовРасходов", ИсключаемыеВидыПрочихДоходовРасходов);
	Запрос.УстановитьПараметр("НачалоПериода", СтрокаПериода.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", Новый Граница(КонецДня(СтрокаПериода.КонецПериода), ВидГраницы.Включая));
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, , , , СчетКт В ИЕРАРХИИ (&Счет), &ВидСубконто, Организация = &Организация) КАК ХозрасчетныйОборотыДтКт
	|ГДЕ
	|	НЕ ВЫРАЗИТЬ(ХозрасчетныйОборотыДтКт.СубконтоКт1 КАК Справочник.ПрочиеДоходыИРасходы).ВидПрочихДоходовИРасходов В (&ИсключаемыеВидыПрочихДоходовРасходов)";
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		СтрокаПоказателя.ЗначениеПоказателя = (Выборка.Сумма);
	КонецЕсли;
	...
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, , , , СчетКт В ИЕРАРХИИ (&Счет), &ВидСубконто, Организация = &Организация) КАК ХозрасчетныйОборотыДтКт
	|ГДЕ
	|	ВЫРАЗИТЬ(ХозрасчетныйОборотыДтКт.СубконтоКт1 КАК Справочник.ПрочиеДоходыИРасходы).ВидПрочихДоходовИРасходов В (&ВидыПрочихДоходовРасходов)";
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() И НЕ Выборка.Сумма = Null Тогда
		СтрокаПоказателя.ЗначениеПоказателя = (Выборка.Сумма);
	КонецЕсли;
...
Функция ЕстьДанныеПоПоказателю(Показатель, ТаблицаСверкиДанных)
	...
	Значение = 0;
	Для Каждого Колонка Из ТаблицаСверкиДанных.Колонки Цикл
		Если ТипЗнч(СтрокаПоказателя[Колонка.Имя]) = Тип("Число") Тогда
			Значение = Значение + СтрокаПоказателя[Колонка.Имя];
		КонецЕсли;
	КонецЦикла;
	Возврат Значение <> 0;
