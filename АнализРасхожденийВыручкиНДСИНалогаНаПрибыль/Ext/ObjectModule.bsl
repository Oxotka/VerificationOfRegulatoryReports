...
Процедура ДополнитьПоказателиУчетнымиДаннымиАктуальныеДанныеДекларацииПоПрибыли(Организация, Показатели, СтрокаПериода)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, , , , СчетКт В ИЕРАРХИИ (&СчетПрочиеДоходы), &ВидСубконто, Организация = &Организация) КАК ХозрасчетныйОборотыДтКт
	|ГДЕ
	...
	|ГДЕ
	|	НЕ ВЫРАЗИТЬ(ХозрасчетныйОборотыДтКт.СубконтоКт1 КАК Справочник.ПрочиеДоходыИРасходы).ВидПрочихДоходовИРасходов В (&ВидыПрочихДоходовРасходов)";
	ТаблицаЗапроса = Запрос.Выполнить().Выгрузить();
	СтрокаПоказателя.ЗначениеПоказателя = ТаблицаЗапроса.Итог("Сумма");
