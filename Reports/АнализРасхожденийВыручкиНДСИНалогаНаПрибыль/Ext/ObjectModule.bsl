...
Процедура ВставитьПоказатели_Прибыль(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2016Кв4"
		...
		НоваяСтрока = ТаблицаПоказателей.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ОписаниеОтчета);
		НоваяСтрока.Показатель = "ВыручкаПоПрочемуИмуществу";
		НоваяСтрока.ЗначениеПоказателя = ЗначениеПоказателя;
	...
	Иначе
		...
		Для Каждого Лист Из СведенияОтчета.ДанныеМногостраничныхРазделов.Лист02_1 Цикл
			ДанныеЛиста = Лист.Данные;
			ЗначениеПоказателя = ЗначениеПоказателя
			                   + ПоказательОтчета(ДанныеЛиста, "П002010001403");
		КонецЦикла;
...
Процедура ДополнитьПоказателиУчетнымиДаннымиАктуальныеДанныеДекларацииПоПрибыли(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
	...
	Запрос.УстановитьПараметр("Счет", ПланыСчетов.Хозрасчетный.ПрочиеДоходы);
	Запрос.УстановитьПараметр("ВидСубконто", ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы);
	ВидыПрочихДоходовРасходовВыручкаПоПрочимОперациям = Новый Массив;
	ВидыПрочихДоходовРасходовВыручкаПоПрочимОперациям.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияОсновныхСредств);
	ВидыПрочихДоходовРасходовВыручкаПоПрочимОперациям.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияНематериальныхАктивов);
	Запрос.УстановитьПараметр("ВидыПрочихДоходовРасходов", ВидыПрочихДоходовРасходовВыручкаПоПрочимОперациям);
	Запрос.УстановитьПараметр("НачалоПериода", СтрокаПериода.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", Новый Граница(КонецДня(СтрокаПериода.КонецПериода), ВидГраницы.Включая));
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	СтрокаПоказателя = Показатели.Добавить();
	СтрокаПоказателя.ИсточникОтчета = "АктуальныеДанные";
	СтрокаПоказателя.Показатель = "АктуальныеВыручкаПоПрочемуИмуществу";
	СтрокаПоказателя.ДатаОтчета = СтрокаПериода.ДатаОтчета;
	СтрокаПоказателя.ДатаОкончания = СтрокаПериода.КонецПериода;
	...
	Запрос.УстановитьПараметр("Счет", ПланыСчетов.Хозрасчетный.ПрочиеДоходы);
	Запрос.УстановитьПараметр("ВидСубконто", ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы);
	ВидыПрочихДоходовРасходовВыручкаПоПрочемуИмуществу = Новый Массив;
	ВидыПрочихДоходовРасходовВыручкаПоПрочемуИмуществу.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияПрочегоИмущества);
	Запрос.УстановитьПараметр("ВидыПрочихДоходовРасходов", ВидыПрочихДоходовРасходовВыручкаПоПрочемуИмуществу);
	Запрос.УстановитьПараметр("НачалоПериода", СтрокаПериода.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", Новый Граница(КонецДня(СтрокаПериода.КонецПериода), ВидГраницы.Включая));
	Запрос.Текст = 
	"ВЫБРАТЬ
...
Функция ТаблицаСверкиДанныхРегламентированныхОтчетов(Показатели, Периоды)
	...
	ДобавитьПоказательОборотный(ТаблицаСверкиДанных, "АктуальныеДанные", "АктуальныеВыручкаПоПрочемуИмуществу", Периоды, Показатели);
	...
	ДобавитьПоказательНакопительный(ТаблицаСверкиДанных, "РегламентированныйОтчетПрибыль", "ВыручкаПоПрочемуИмуществу", Периоды, Показатели);
...
Процедура ЗаполнитьТабличныйДокумент(ТабличныйДокумент, ТаблицаСверкиДанных, Периоды, Макет)
	...
	Если ЕстьДанныеПоПоказателю("ВыручкаПоПрочемуИмуществу", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "ВыручкаПоПрочемуИмуществу", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,,Истина);
	КонецЕсли;
