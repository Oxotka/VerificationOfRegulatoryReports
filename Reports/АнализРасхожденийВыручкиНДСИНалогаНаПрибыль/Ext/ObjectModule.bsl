...
Функция СформироватьОтчет(Параметры) Экспорт
	...
	Если Показатели.Количество() = 0 Тогда
		...
		СтруктураРезультата.Вставить("ПараметрыРасшифровки", Новый Структура);
	...
	ДополнитьПоказателиУчетнымиДанными(Организация, МассивСубконтоПДР, Показатели, Периоды, ПараметрыРасшифровки);
	ДополнитьПараметрыРасшифровкиПоНакопительнымПериодам(ПараметрыРасшифровки, Периоды);
	...
	СтруктураРезультата = Новый Структура;
	СтруктураРезультата.Вставить("ТабличныйДокумент", ТабличныйДокумент);
	СтруктураРезультата.Вставить("ПараметрыРасшифровки", ПараметрыРасшифровки);
	Возврат СтруктураРезультата;
...
Функция ОтчетыСверкиДанныхРегламентированныхОтчетов(Организация, ДатаНачала, ДатаОкончания)
	...
	Пока Выборка.Следующий() Цикл
		...
		// У декларации по налогу на прибыль есть более точный признак Головной организации.
		// Проверим его.
		ЭтоГоловнаяОрганизация = Истина;
		Если Выборка.ИсточникОтчета = "РегламентированныйОтчетПрибыль" Тогда
			ДанныеОтчета = Выборка.РегламентированныйОтчет.ДанныеОтчета.Получить();
			Если ДанныеОтчета.Свойство("ДанныеМногостраничныхРазделов") И ДанныеОтчета.ДанныеМногостраничныхРазделов.Лист02_5.Количество() > 0 Тогда
				ЗначениеЯчейки = ДанныеОтчета.ДанныеМногостраничныхРазделов.Лист02_5[0].Данные.П000250001000;
				Если Не (ЗначениеЯчейки = "" Или ЗначениеЯчейки = "1") Тогда
...
Процедура ВставитьПоказатели_Прибыль(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2016Кв4"
		...
		Для Каждого Лист Из СведенияОтчета.ДанныеМногостраничныхРазделов.Лист02_1 Цикл
			ДанныеЛиста = Лист.Данные;
			ЗначениеПоказателя = ЗначениеПоказателя
			                   + ПоказательОтчета(ДанныеЛиста, "П000210001403");
		КонецЦикла;
		
		НоваяСтрока = ТаблицаПоказателей.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ОписаниеОтчета);
		НоваяСтрока.Показатель = "ВыручкаПоРеализацииПрочегоИмущества";
		НоваяСтрока.ЗначениеПоказателя = ЗначениеПоказателя;
		
	ИначеЕсли ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2019Кв4" 
		Или ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2020Кв4" Тогда 
		...
		Для Каждого Лист Из СведенияОтчета.ДанныеМногоуровневыхРазделов.Лист02.Строки Цикл
			ДанныеЛиста = Лист.Данные;
			ЗначениеПоказателя = ЗначениеПоказателя
			+ ПоказательОтчета(ДанныеЛиста, "П000020002003");
		КонецЦикла;
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Внимание! Расширение не адаптировано для работы с новой формой декларации по налогу на прибыль - возможны ошибки.
			|Пожалуйста, обратитесь за обновлением в техподдержку'");
...
Процедура ВставитьПоказатели_НДС(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Иначе
		ТекстСообщения = НСтр("ru='Внимание! Расширение не адаптировано для работы с новой декларацией НДС - возможны ошибки.
			|Пожалуйста, обратитесь за обновлением в техподдержку.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
Процедура ДополнитьПоказателиУчетнымиДанными(Организация, МассивСубконтоПДР, Показатели, Периоды, ПараметрыРасшифровки)
	
	ПараметрыРасшифровки.Вставить("ИменаПоказателейДляСложныхПериодов", Новый Массив);
	
	Для Каждого СтрокаПериода Из Периоды Цикл
		
		ДополнитьПоказателиУчетнымиДаннымиНеподтвержденныйНДС(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		
		Если СтрокаПериода.Накопительный Тогда
			Продолжить;
		КонецЕсли;
		
		ДополнитьПоказателиУчетнымиДаннымиВозвратПоставщику(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиВозвратОтПокупателя(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиКорректировкаРеализации(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиВнереализационныеДоходыНеОблагаемыеНДС(Организация, Показатели, МассивСубконтоПДР, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиДоходыНеоблагаемыеНДС(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиДоходыЕНВД(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиНеПодтвержденнаяРеализация0Совсем(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиАктуальныеДанныеДекларацииПоПрибыли(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиБезвозмезднаяПередача(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиОтгрузкаБезПереходаПраваСобственности(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		ДополнитьПоказателиУчетнымиДаннымиРеализацияОтгруженныхТоваров(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ДополнитьПоказателиУчетнымиДаннымиАктуальныеДанныеДекларацииПоПрибыли(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
	...
	//Такие доходы добавляем к выручке по обычным видам деятельности и исключаем из внереализационых доходов
	...
	ВидыПрочихДоходовРасходов.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияПрочегоИмущества);
	...
	Запрос.УстановитьПараметр("Счет", ПланыСчетов.Хозрасчетный.ПрочиеДоходы);
	Запрос.УстановитьПараметр("ВидСубконто", ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы);
	ВидыПрочихДоходовРасходовПроичеОперации = Новый Массив;
	ВидыПрочихДоходовРасходовПроичеОперации.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияОсновныхСредств);
	ВидыПрочихДоходовРасходовПроичеОперации.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияНематериальныхАктивов);
	Запрос.УстановитьПараметр("ВидыПрочихДоходовРасходов", ВидыПрочихДоходовРасходовПроичеОперации);
	Запрос.УстановитьПараметр("НачалоПериода", СтрокаПериода.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", Новый Граница(КонецДня(СтрокаПериода.КонецПериода), ВидГраницы.Включая));
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	СтрокаПоказателя = Показатели.Добавить();
	СтрокаПоказателя.ИсточникОтчета = "АктуальныеДанные";
	СтрокаПоказателя.Показатель = "АктуальныеВыручкаПоРеализацииПрочегоИмущества";
	СтрокаПоказателя.ДатаОтчета = СтрокаПериода.ДатаОтчета;
	СтрокаПоказателя.ДатаОкончания = СтрокаПериода.КонецПериода;
	...
	Запрос.УстановитьПараметр("ВидСубконто", ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы);
	//Такие доходы добавляем к выручке по обычным видам деятельности и исключаем из внереализационых доходов
	ВидПрочихДоходовРасходовРеализацияПрочегоИмущества = Новый Массив;
	ВидПрочихДоходовРасходовРеализацияПрочегоИмущества.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.РеализацияПрочегоИмущества);

	Запрос.УстановитьПараметр("ВидыПрочихДоходовРасходов", ВидПрочихДоходовРасходовРеализацияПрочегоИмущества);
	Запрос.УстановитьПараметр("НачалоПериода", СтрокаПериода.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", Новый Граница(КонецДня(СтрокаПериода.КонецПериода), ВидГраницы.Включая));
	Запрос.Текст = 
	"ВЫБРАТЬ
...
Процедура ДополнитьПоказателиУчетнымиДаннымиВозвратПоставщику(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
	...
	|	НДСЗаписиКнигиПродажОбороты.Покупатель КАК Информация,
	|	НДСЗаписиКнигиПродажОбороты.СчетФактура КАК Документ
	|ИЗ
	...
	|			,
	|			Организация = &Организация
	|				И СчетФактура ССЫЛКА Документ.ВозвратТоваровПоставщику
	...
	СтрокаПоказателя.ЗначениеПоказателя = Таблица.Итог("Сумма");
	
	ИмяПараметраРасшифровка = СтрШаблон("%1%2", СтрокаПоказателя.Показатель, СтрокаПериода.ИмяКолонки);
	МассивСтруктур = ОбщегоНазначения.ТаблицаЗначенийВМассив(Таблица);
	ПараметрыРасшифровки.Вставить(ИмяПараметраРасшифровка, МассивСтруктур);
	ПараметрыРасшифровки.ИменаПоказателейДляСложныхПериодов.Добавить(СтрокаПоказателя.Показатель);
	
КонецПроцедуры

Процедура ДополнитьПоказателиУчетнымиДаннымиВозвратОтПокупателя(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
	...
	|	НДСПредъявленныйОбороты.СчетФактура КАК Документ,
	|	НДСПредъявленныйОбороты.Поставщик КАК Информация
	|ИЗ
...
Процедура ДополнитьПоказателиУчетнымиДаннымиВнереализационныеДоходыНеОблагаемыеНДС(Организация, Показатели, МассивСубконтоПДР, СтрокаПериода, ПараметрыРасшифровки)
	...
	|	ХозрасчетныйОбороты.Субконто1 КАК Информация
	|ИЗ
...
Процедура ДополнитьПоказателиУчетнымиДаннымиНеподтвержденныйНДС(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
	...
	Если СтрокаПериода.Накопительный Тогда
		...
		|	НДСРеализация0Обороты.СчетФактура КАК Документ,
		|	НДСРеализация0Обороты.Покупатель КАК Информация
		|ИЗ
		...
		ИмяПараметраРасшифровка = СтрШаблон("%1%2", "НеподтвержденныйНДС", СтрокаПериода.ИмяКолонки);
		МассивСтруктур = ОбщегоНазначения.ТаблицаЗначенийВМассив(Таблица);
		ПараметрыРасшифровки.Вставить(ИмяПараметраРасшифровка, МассивСтруктур);
	Иначе
		СтрокаПоказателя = Показатели.Добавить();
...
Процедура ДополнитьПоказателиУчетнымиДаннымиКорректировкаРеализации(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
	...
	|				И НАЧАЛОПЕРИОДА(СчетФактура.Дата, ГОД) = НАЧАЛОПЕРИОДА(СчетФактура.ДокументРеализации.Дата, ГОД)) КАК НДСПредъявленныйОбороты";
...
Процедура ДополнитьПоказателиУчетнымиДаннымиДоходыНеоблагаемыеНДС(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
...
Процедура ДополнитьПоказателиУчетнымиДаннымиДоходыЕНВД(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
	...
	|	ХозрасчетныйОбороты.Субконто1 КАК Информация
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, Регистратор, Счет В (&Счета90012), , Организация = &Организация, , ) КАК ХозрасчетныйОбороты";
	Таблица = Запрос.Выполнить().Выгрузить();
	СтрокаПоказателя.ЗначениеПоказателя = Таблица.Итог("Сумма");
...
Процедура ДополнитьПоказателиУчетнымиДаннымиНеПодтвержденнаяРеализация0Совсем(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
...
Процедура ДополнитьПоказателиУчетнымиДаннымиБезвозмезднаяПередача(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
...
Процедура ДополнитьПоказателиУчетнымиДаннымиОтгрузкаБезПереходаПраваСобственности(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
...
Процедура ДополнитьПоказателиУчетнымиДаннымиРеализацияОтгруженныхТоваров(Организация, Показатели, СтрокаПериода, ПараметрыРасшифровки)
...
Функция ТаблицаСверкиДанныхРегламентированныхОтчетов(Показатели, Периоды)
	...
	ДобавитьПоказательОборотный(ТаблицаСверкиДанных, "АктуальныеДанные", "АктуальныеВыручкаПоРеализацииПрочегоИмущества", Периоды, Показатели);
	...
	ДобавитьПоказательНакопительный(ТаблицаСверкиДанных, "РегламентированныйОтчетПрибыль", "ВыручкаПоРеализацииПрочегоИмущества", Периоды, Показатели);
...
Процедура ДополнитьПараметрыРасшифровкиПоНакопительнымПериодам(ПараметрыРасшифровки, Периоды)
	...
	Для Каждого ПериодДляРасчета Из МассивПериодовДляРасчета Цикл
		...
		Если ПериодДляРасчета = "Полугодие" Тогда
			ИмяПериода1 = "Квартал1";
			ИмяПериода2 = "Квартал2";
		ИначеЕсли ПериодДляРасчета = "Месяцев9" Тогда
			ИмяПериода1 = "Полугодие";
			ИмяПериода2 = "Квартал3";
		ИначеЕсли ПериодДляРасчета = "Год" Тогда
			ИмяПериода1 = "Месяцев9";
			ИмяПериода2 = "Квартал4";
		КонецЕсли;
		
		Для Каждого ИмяПоказателя Из ПараметрыРасшифровки.ИменаПоказателейДляСложныхПериодов Цикл
			Слагаемое1 = ПараметрыРасшифровки[ИмяПоказателя + ИмяПериода1];
			Слагаемое2 = ПараметрыРасшифровки[ИмяПоказателя + ИмяПериода2];
			МассивСтруктур = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(Слагаемое1);
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСтруктур, Слагаемое2);
			ИмяПараметраРасшифровка = СтрШаблон("%1%2", ИмяПоказателя, ПериодДляРасчета);
			ПараметрыРасшифровки.Вставить(ИмяПараметраРасшифровка, МассивСтруктур);
		КонецЦикла;
	КонецЦикла;
...
Процедура ЗаполнитьТабличныйДокумент(ТабличныйДокумент, ТаблицаСверкиДанных, Периоды, Макет)
	...
	Если ЕстьДанныеПоПоказателю("ВыручкаПоРеализацииПрочегоИмущества", ТаблицаСверкиДанных) Тогда
		Область = Макет.ПолучитьОбласть("СтрокаНижнийУровеньКурсив|Показатель");
		Область.Параметры.Показатель = "Выручка от реализации прочего имущества" + Символы.ПС + "Приложение №1 к листу 02, строка 014";
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "ВыручкаПоРеализацииПрочегоИмущества", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("НеподтвержденныйНДС", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "НеподтвержденныйНДС", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("ВозвратПоставщику", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "ВозвратПоставщику", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("ВозвратОтПокупателя", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "ВозвратОтПокупателя", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("КорректировкаРеализации", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "КорректировкаРеализации", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("ДоходыНеоблагаемыеНДС", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "ДоходыНеоблагаемыеНДС", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("ДоходыЕНВД", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "ДоходыЕНВД", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("ВнерелизационныеДоходыНеОблагаемыеНДС", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "ВнерелизационныеДоходыНеОблагаемыеНДС", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("НеподтвержденныйНДС0", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "НеподтвержденныйНДС0", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("БезвозмезднаяПередача", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "БезвозмезднаяПередача", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("ОтгрузкаБезПереходаПраваСобственности", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "ОтгрузкаБезПереходаПраваСобственности", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
	...
	Если ЕстьДанныеПоПоказателю("РеализацияОтгруженныхТоваров", ТаблицаСверкиДанных) Тогда
		...
		ТабличныйДокумент.Вывести(Область);
		ВывестиЗначенияПоказателей(Макет, Периоды, "РеализацияОтгруженныхТоваров", ТаблицаСверкиДанных, ТабличныйДокумент, Истина,,Ложь,,Истина);
	КонецЕсли;
...
Процедура ВывестиЗначенияПоказателей(Макет, Периоды, Показатель, ТаблицаСверкиДанных, ТабличныйДокумент, ЭтоРасшифровка = Ложь, Оборотный = Ложь, ВыделятьЦветом = Истина, ПроверитьАктуальныеДанные = Ложь, ЭтоРасшифровкаЗначения = Ложь)
	
	Если ЭтоРасшифровкаЗначения Тогда
		Область = Макет.ПолучитьОбласть("СтрокаНижнийУровеньРасшифровка|Период");
	...
	Для Каждого СтрокаПериода Из Периоды Цикл
		...
		КонецЕсли;
		Если ЭтоРасшифровкаЗначения Тогда
			Область.Параметры.Расшифровка = Новый Структура("ИмяТаблицы", СтрШаблон("%1%2", Показатель, СтрокаПериода.ИмяКолонки));
		КонецЕсли;
